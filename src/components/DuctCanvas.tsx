...
export function DuctCanvas({
  ...
  function handleMouseUp(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    if (seg1) newSegs.push({ id: uid(), ...seg1, size, cfm, layer: currentLayer, isReturn });
    if (seg2) newSegs.push({ id: uid(), ...seg2, size, cfm, layer: currentLayer, isReturn });

    // Validate: no interior-wall crossings, no duct through diffuser interiors
    for (const seg of newSegs) {
      if (segCrossesInteriorWall(seg, level.floorplan)) {
        errorFlash.current = Date.now();
        s.drawStart = null;
        render();
        setTimeout(() => render(), 520);
        return;
      }
      if (segPassesThroughDiffuser(seg, ductSystem.diffusers)) {
        errorFlash.current = Date.now();
        s.drawStart = null;
        render();
        setTimeout(() => render(), 520);
